...
def getOptions():
    ...
    parser = argparse.ArgumentParser(description="Pathway Enrichment",
	   formatter_class=RawDescriptionHelpFormatter)

    standard = parser.add_argument_group(description="Input")

    standard.add_argument('-k',"--kegg", dest="kegg_downloader", action='store', 
	   required=True, help="Kegg Downloader Output File.")
    standard.add_argument('-s',"--steps", dest="steps_away", action='store', 
	   required=True, help="Number of steps away from desired meatbolite.")
    
    standard.add_argument('-g',"--gene", dest="deaGeneDataset", action='store', 
	   required=True, help="Differential Expression Analysis Gene Dataset.")
    standard.add_argument('-gid',"--gene_id", dest="gene_id_col", action='store', 
	   required=True, help="DEA Gene Dataset Unique Identificator column.")
    standard.add_argument('-gf',"--gene_flag", dest="gene_flag_col", action='store', 
	   required=True, help="DEA Gene Dataset Flag column.")
    
    standard.add_argument('-m',"--met", dest="deaMetDataset", action='store', 
	   required=True, help="Differential Expression Analysis Metabolomic Dataset.")
    standard.add_argument('-mid',"--met_id", dest="met_id_col", action='store', 
	   required=True, help="DEA Metabolomic Dataset Unique Identificator column.")
    standard.add_argument('-mf',"--met_flag", dest="met_flag_col", action='store', 
	   required=True, help="DEA Metabolomic Dataset Flag column.")   
...
def createDict(args):
    ...
    """
    Function that creates a dictionary with the association between different pathways and the features involved in it,
...
def pathwayFisher(genes1step, step):
    ...
    output.write("Genes" + str(step) + "stepAway\tDEG_with_DEM\tNoDEG_with_DEM\tDEG_with_NoDEM\tNoDEG_with_NoDEM\tPvalue\tOddsRatio\n")
            ...
            if "*" in metabolite:
                ...
                for gene in genes1step[metabolite]:
                    if "*" in gene:
                        ...
                        NoDEM_NoDEG += 1
        oddsratio, pvalue = st.fisher_exact([[DEM_DEG, DEM_NoDEG], [NoDEM_DEG, NoDEM_NoDEG]])
        output.write(str(numberOfGenes) + "\t" + str(DEM_DEG) + "\t" + str(DEM_NoDEG) + "\t" + str(NoDEM_DEG) + "\t" + str(NoDEM_NoDEG) + "\t" + str(pvalue) + "\t" + str(oddsratio) + "\n")
...
#    PEAList = modules.fisherExactTest(args, pathNames)
