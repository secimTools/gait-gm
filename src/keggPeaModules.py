...
def checkForDuplicates(dataset, uniqID):
    ...
    """
    This function checks for duplicated values in a dataset column. If found, it terminates the program giving extra info
    in the stderr file
    ...
    if not duplicates.empty:
        ...
        sys.stderr.write("Duplicated IDs!\n\nRow_Number\tRepeated_ID\n" + str(duplicates))
        sys.exit(1)

############################
# Add KEGG Annotation Info #
############################

def downloadGeneParser(species):
    
    """
    Download gene information (kegg_id, gene_name) from KEGG database.
...
def keggAnno(feature_table, feature2keggs, featureOut, uniqIDNameCol, featureNameCol, featureType):
    ...
    """
    Function that takes the Name column of a Dataset and finds KEGG-related information.
    It works for Gene Expression and Metabolomic data (depending on featureType parameter).
    It will return an Annotation Dataset with this information and it will inform about possible ties.
    ...
    for index, featureRow in feature_table.iterrows():
        ...
        if featureName.lower() in emptyList:
            tmpList_v1.append(uniqueID + "\t" + featureName + "\t" + featureType + "\tNA\tNA\tNA\tNA\tNA\tNA\n")
        else:
            featureDict_v1 = {}
            ...
            for kegg_id, keggFeatureNames in feature2keggs.items():
                if featureType == "Gene":
                    ...
                    for keggGeneName in keggGeneNames:
                        if re.search(".*" + re.escape(str(featureName)) + ".*", keggGeneName, re.IGNORECASE):
                            similarity = calculateSimilarity(str(featureName).strip(), str(keggGeneName).strip())
                            featureDict_v1[str(keggGeneName).strip()] = [similarity, kegg_id]
                else:
                    for newMetName in newMetNames:
                        if re.search(".*" + re.escape(newMetName) + ".*", keggFeatureNames, re.IGNORECASE):
                            featureDict_v1 = add2Dictionary(featureName, newMetName, kegg_id, keggFeatureNames, featureDict_v1) 
            if featureDict_v1:
                ...
                if len(featureDict_v1) > 1:
                    ...
                    if (maximum_v1[0] == maximum_v2[0]) or (maximum_v1[0] - maximum_v2[0] <= 0.05*maximum_v1[0]):
                        # Warning Message
                        print("Warning! There is a tie with", featureName + ":", list(featureDict_v1.keys())[list(featureDict_v1.values()).index(maximum_v1)], "selected,", list(featureDict_v1.keys())[list(featureDict_v1.values()).index(maximum_v2)], "rejected.")
                        is_tie = "Yes"
                    else:
                        is_tie = "No"
                else:
                ...
                tmpList_v1.append(uniqueID + "\t" + featureName + "\t" + featureType + "\t" + "Yes" + "\t" + str(list(featureDict_v1.keys())[list(featureDict_v1.values()).index(maximum_v1)]) + "\t" + str(maximum_v1[1]) + "\t" + str(round(maximum_v1[0], 2)) + "\t" + is_tie + "\tYes\n")                           
                featureDict_v2 = dict(featureDict_v1)
                del featureDict_v2[list(featureDict_v2.keys())[list(featureDict_v2.values()).index(maximum_v1)]]
                for keggName in featureDict_v2:
                    if is_tie == "Yes":
                        if keggName == list(featureDict_v1.keys())[list(featureDict_v1.values()).index(maximum_v2)]:
                            tmpList_v1.append(uniqueID + "\t" + featureName + "\t" + featureType + "\tYes\t" + str(keggName.strip()) + "\t" + str(featureDict_v2[keggName][1]) + "\t" + str(round(featureDict_v2[keggName][0], 2)) + "\t" + is_tie + "\tNo\n")
                        else:
                            tmpList_v2.append(uniqueID + "\t" + featureName + "\t" + featureType + "\tYes\t" + str(keggName.strip()) + "\t" + str(featureDict_v2[keggName][1]) + "\t" + str(round(featureDict_v2[keggName][0], 2)) + "\tNo\tNo\n")
                    else:
                        tmpList_v2.append(uniqueID + "\t" + featureName + "\t" + featureType + "\tYes\t" + str(keggName.strip()) + "\t" + str(featureDict_v2[keggName][1]) + "\t" + str(round(featureDict_v2[keggName][0], 2)) + "\t" + is_tie + "\tNo\n")
            else:
                tmpList_v1.append(uniqueID + "\t" + featureName + "\t" + featureType + "\tNo\tNA\tNA\tNA\tNA\tNA\n")
...
def metaboliteModification(metabolite):
    ...
    """
    Modify input metabolite name in order to find its kegg identifier. It will be modified following this pipeline:
   ...
   Arguments:
    ...
    mainPrefixes = [re.compile(r"^n?(-|\s)?[0-9]?(-|\s)?.*yl(-|\s)"), re.compile(r"^(((l|d|dl|ld)|[0-9])(-|\s))?tert(-|\s)?"), re.compile(r"^[0-9]?(-|\s)?hydroxy(-|\s)?"),
                    re.compile(r"^n?(-|\s)?[0-9]?(-|\s)?boc(-|\s)?"), re.compile(r"\(.*\)"), re.compile(r"^n?(-|\s)?[0-9]?(-|\s)?acetyl(-|\s)?"),
                     re.compile(r"^((cis|trans)?(-|\s))?((s|r|\(s\)|\(r\))?(-|\s))?((([0-9]?)(,?))+?(-|\s))?(n(-|\s))?((d|l|dl|ld)(-|\s))?((alpha|beta|a|b)(-|\s))?(([0-9]?)(,?))+?(-|\s)?")]
    # Add as many chemical words as desired
    chemWords = ["sulfoxide", "-sulfoxide", "sulfate",  "-sulfate", "sulfonate", "-sulfonate"]
    # Commonly abbreviated words, like aminoacids
    aminoacids = {'l-cysteine': ['c', 'cys'], 'l-aspartate': ['d', 'asp'], 'l-serine': ['s', 'ser'], 'l-glutamine': ['q', 'gln'],
              'l-lysine': ['k', 'lys'], 'l-isoleucine': ['i', 'ile'], 'l-proline': ['p', 'pro'], 'l-threonine': ['t', 'thr'],
              'l-phenylalanine': ['f', 'phe'], 'l-asparagine': ['n', 'asn'], 'glycine': ['g', 'gly'], 'l-histidine': ['h', 'his'],
              'l-leucine': ['l', 'leu'], 'l-arginine': ['r', 'arg'], 'l-tryptophan': ['w', 'trp'], 'l-alanine': ['a', 'ala'],
              'l-valine': ['v', 'val'], 'l-glutamate': ['e', 'glu'], 'l-tyrosine': ['y', 'tyr'], 'l-methionine': ['m', 'met']}
    abbreviations = {'citrate': 'cit', 'ornithine': 'orn', 'thyroxine': 'thyr', 'butoxycarbonyl': 'boc'}  
    lipids = {'sphingomyelin': 'sm', 'lysophosphatidylcholine': 'lysopc', 'phosphatidylcholine': 'pc', 'phosphatidylethanolamine': 'pe',
    ...
    if (metabolite in dict(tuple(aminoacids.values())).keys()) or (metabolite in dict(tuple(aminoacids.values())).values()) or (metabolite in str(tuple(aminoacids.keys()))):
    ...
    if (any(completeName in metabolite for completeName in abbreviations.values())):
        for completeName in abbreviations.keys():
            if (abbreviations[completeName] in metabolite):
                newMetNames.append(re.sub(abbreviations[completeName], completeName, metabolite))
                newMetNames.append(metabolite)
    # If metabolite is an acid, use the -ate nomenclature
    if (("ic acid" in metabolite) or ("ic_acid" in metabolite) or ("icacid" in metabolite)) and (re.sub(r"ic.?acid", "ate", metabolite) not in newMetNames):
    ...
    if any(chemWord in metabolite for chemWord in chemWords):
        for chemWord in chemWords:
            if (chemWord in metabolite) and (re.sub(chemWord, "", metabolite) not in newMetNames):
                newMetNames.append(re.sub(chemWord, "", metabolite)) 
                newMetNames.append(metabolite)
    # Search in pubchem for synonyms - skipped
...
#                    newMetNames.append(str(synonym))
    ...
    for newMetName in newMetNames:
        for mainPrefix in mainPrefixes:
            if mainPrefix.search(newMetName) and (re.sub(mainPrefix, "", newMetName) not in newMetNamesNoPrefix) and (newMetName not in aminoacids):
...
def add2Dictionary(metabolite, newMetName, kegg_cpd, keggMetNames, metDict):
    ...
    for keggMetName in keggMetNames:
        if re.search(".*" + re.escape(newMetName) + ".*", keggMetName.strip(), re.IGNORECASE):
...
def calculateSimilarity(featureName, keggName):
    ...
    """
    Function to compare two feature (gene/metabolite) names and return the similarity between them.
    If the difference between them is only one of these mainPrefixes and/or only differs in the case,
    a similarity of 90% is returned.
    ...
    mainPrefixes = ["", "cis-","trans-","d-","l-","(s)-","alpha-","beta-","alpha ","beta ",
    "alpha-d-","beta-d-","alpha-l-","beta-l-", "l-beta-", "l-alpha-", "d-beta-", "d-alpha-"]
...
def downloadKeggInfo(args):
    ...
    if args.geneKeggAnnot:
        urllib.request.urlretrieve("http://rest.kegg.jp/link/" + args.species + "/pathway", args.kgen2pathways)
        
    # MetaboliteKeggID2PathwayID
    if args.metKeggAnnot:
        urllib.request.urlretrieve("http://rest.kegg.jp/link/compound/pathway", args.kmet2pathways)
        
    # PathwayID2PathwayNames
    urllib.request.urlretrieve("http://rest.kegg.jp/list/pathway/" + args.species, args.pathways)  
...
def keggAnnot2list(keggAnnotFile, UniqueID, featureName, featureKeggId, featureType):
    ...
    """
    This function is used when the input dataset has been parsed with metabolite_parser tool.
    This tool creates a dictionary that would be used in the next function to find KEGG pathway information.
    ...
    with open(keggAnnotFile, "r") as keggAnnot:
        ...
        for line in keggAnnot:
            ...
            if 'Selected' in header:
                ...
                if selected == "NA":
                    featureList.append(uniqueID + "\t" + featureName + "\t" + featureType + "\t" + "NA" + "\t" + "NA" + "\t" + "NA" + "\n")
            else:
                kegg_id = line.split("\t")[keggIdCol]
                if kegg_id:
                    featureDict[uniqueID + "\t" + featureName] = kegg_id
...
def add_path_info(featureDict, featureList, featureType, keggId2pathway, pathId2pathName, species, outputFile):
    
    """
    Function to find all pathways in kegg related to a gene or a metabolite.
    ...
    output.write("UniqueID\tFeature_Name\tFeature_Type\tKEGG_ID\tPathway_ID\tPathway_Name\n")
...
def parseWord(toParseList, parser, nameIndex, species):
    ...
    """
    Function that looks into a column of a kegg file, finds a given word and returns the value of the other column.
    Useful to parse genes and metabolites into kegg identifiers and find related pathways with its names.
...
def atoi(text):
    ...
    """
    This 2 tools sort numbers in human order.
    """
...
def Ids2Names(dataset, Id, annot, annotName):
    ...
    """
    This function change unique identifiers by feature names. It could be used for genes or for metabolites.
...
def prepareSPLSData(args):
    ...
    """
    This function preforms the subsetting of the data for the sPLS tool.
    ...
    for keggName, uniqueIDs in metSubsetDict.items():
        if 1 < len(uniqueIDs) < 3:
            print("Metabolite", keggName.split(";")[0], "has only", len(uniqueIDs), "repetitions. At least 3 repetitions are needed to perform an sPLS.")
            ...
            for uniqueID in uniqueIDs:
                for index, row in metTable.iterrows():
                    if uniqueID == row[args.metId]:
                        metSubTable = metSubTable.append(pd.Series(row, index=metTable.columns), ignore_index=True)
            # Add Annotation
            if args.metAnno:
                metSubTable = Ids2Names(metSubTable, args.metId, args.metAnno, args.metName)
            else:
...
def prepareSPLSMetGenericData(args):
    ...
    """
    This function is specific for Subsetting by Generic Metabolite in the sPLS Tool.
...
def prepareSPLSMetMMCData(args):
    ...
    """
    This function is specific for Subsetting by MMC clusters in the sPLS Tool.
    ...
    MMCResultTable = pd.DataFrame(columns = [args.metId, 'Module', 'Entry Index', 'Average Degree', 'Degree'])
...
def prepareSPLSMetBothData(args):
    ...
    """
    This function is specific for Subsetting by Generic Metabolite and MMC clusters in the sPLS Tool.
    ...
    metSubsetDict = {}
    fh1List, fh2List, fh3List = [], [], []
    MMCResultTable = pd.DataFrame(columns = [args.metId, 'Module', 'Entry Index', 'Average Degree', 'Degree', 'Subset'])
    ...
    for keggName, uniqueIds in genericMetsDict.items():
        if 1 < len(uniqueIds) < 3:
            ...
            args.uniqID = args.metId
            mmc_table, fh1, fh2, fh3, MMCResultTable = mmc_main(args, keggName.split(';')[0], MMCResultTable)
            fh1List.append(fh1)
            fh2List.append(fh2)
            fh3List.append(fh3)
            mmc_tables = mmc_table.groupby("Module", as_index=False)
            mmc_dict = dict(iter(mmc_tables))
            for module, data in mmc_dict.items():
                metSubsetDict[keggName.split(';')[0] + '_Module_' + str(module)] = data[args.metId].tolist()
            temp.close()
...
def prepareSPLSGeneAllData(args):
    ...
    """
    This function is specific for Subsetting by All Genes (no subseting) in the sPLS Tool.
...
def prepareSPLSGeneListData(args):
    ...
    """
    This function is specific for Subsetting by geneList in the sPLS Tool.
    ...
    # Add Annotation
    if args.geneAnno:
        geneTable_subset = Ids2Names(geneTable_subset, args.geneId, args.geneAnno, args.geneName)
    else:
...
def prepareSPLSGenePathData(args, uniqueIDs):
    ...
    """
    This function is specific for Subsetting by metabolite's pathways in the sPLS Tool.
    ...
    pathsMet = metKeggPath.loc[(metKeggPath[args.metId].isin(uniqueIDs)), 'Pathway_Name'].drop_duplicates().values.tolist()
    # Avoid NA:
    ...
    if not pathsMet:
        ...
        genesInPathsMet = geneKeggPath.loc[(geneKeggPath['Pathway_Name'].isin(pathsMet)), args.geneId].drop_duplicates().values.tolist()   
        geneTable_subset = args.geneDataset.loc[(args.geneDataset[args.geneId].isin(genesInPathsMet))]
        # Add Annotation
        if args.geneAnno:
            geneTable_subset = Ids2Names(geneTable_subset, args.geneId, args.geneAnno, args.geneName)
        else:
...
def prepareSPLSGenePanaData(args):
    ...
    """
    This function is specific for Subsetting by metagenes (PANA) in the sPLS Tool.
    ...
    pathway2genes = pd.read_table(args.path2genes, sep = "\t", header=None, names=['pathId', 'geneId'])
    genes2pathway = pathway2genes[['geneId', 'pathId']]
    # convert to R dataframe
    R_genes2pathway = pandas2ri.py2ri(genes2pathway)
    
    # Run PANA script    
    panaOutput = PANAScript.PCA2GO(X = R_input_file, annotation = R_genes2pathway, var_cutoff = args.cutoff, fac_sel = args.facSel)
    ...
    if args.path2names:
        ...
        path2name = pd.read_table(args.path2names, sep = "\t", names = ['pathId', 'pathName'])
        path2nameDict = pd.Series(path2name.pathName.values, index=path2name.pathId).to_dict()
        pathNames = []
        ...
        if args.geneKeggAnno:
            panaOutputTable = Ids2Names(panaOutputTable, 'KEGG_ID', args.geneKeggAnno, args.geneKeggName)
    else:
        gene_df = pandas2ri.ri2py(panaOutput[0])
        gene_df.set_index("metagene_name", inplace = True)
        R_gene_df = pandas2ri.py2ri(gene_df)
...
def reduce_path_name(pathway, path2nameDict):
    ...
    """
    This function takes a pathway name from KEGG and removes spaces and vowels to save space. Specially useful
...
####################################################
# The following 4 functions have been modified from:
################################################################################
# DATE: 2017/01/17
#
...
def mmc_main(args, outputName, MMCResultTable):
    ...
    mmc_table = mmc_nontechnical_analysis(args, dat.wide, mask, C, clustering, outputName, MMCResultTable)
...
def mmc_nontechnical_analysis(args, df, mask, C, clustering, outputName, MMCResultTable):
    ...
    if args.metOption == 'both':
        header = (args.metId, 'Module', 'Entry Index', 'Average Degree', 'Degree', 'Subset')
    else:
    ...
    if args.metAnno:
        ...
        for noSortName in remaining_row_names:
            newName = annotTable.loc[annotTable[args.metId] == noSortName, args.metName].item()
            newNames.append(noSortName + ": " + newName)
        for sortName in hpnames:
            newName = annotTable.loc[annotTable[args.metId] == sortName, args.metName].item()
    ...
    plotHeatmap(C,fh1.ax[0], cmap=palette.mpl_colormap, xlbls=newNames,
                    ylbls=newNames)
                    
    # Draw second heatmap [C_sorted](reordered according to the clustering).
    plotHeatmap(C_sorted,fh2.ax[0], cmap=palette.mpl_colormap, xlbls=sortNewNames,
                    ylbls=sortNewNames)
    
    # Draw the heatmap [C_smoothed](smoothed version of C_sorted)
    plotHeatmap(C_smoothed,fh3.ax[0], cmap=palette.mpl_colormap, xlbls=sortNewNames,
                    ylbls=sortNewNames)                
    
    if args.metOption == 'both':
        fh1.formatAxis(xTitle="sampleID", figTitle="Correlations for: " + outputName)
        fh2.formatAxis(xTitle="sampleID", figTitle="Re-Ordered correlations for: " + outputName)
        fh3.formatAxis(xTitle="sampleID", figTitle="Smoothed correlations for: " + outputName)
    else:
...
def mmcPlot(figurePath, fh1List, fh2List, fh3List):
    ...
    """
    This function creates a pdf file and writes all the MMC plots in it.
...
def mmcWriteOutput(args, MMCTable):
    ...
    """
    This function writes the MMC Output table in a file. It's possible to change the IDs for Names in an annotation file.
