...
def getOptions():
    ...
    parser = argparse.ArgumentParser(description="split_wide_dataset", formatter_class=RawDescriptionHelpFormatter)
    ...
    standard.add_argument('-id',"--ID", dest="uniqID", action='store', 
            required=False, help="Name of the column with unique identifiers.") 
            
    # Tool Input
    tool = parser.add_argument_group(title='Tool Specific Inputs')
    tool.add_argument('-s',"--samples", dest="samples", action='store',
                      required=True, help="Select sample columns.")
    tool.add_argument('-p',"--prefix", dest="prefix", action='store',
                      required=False, help="Prefix to add to the new unique ID.")
    tool.add_argument('-p2',"--prefix2", dest="prefix2", action='store',
                      required=False, help="Prefix to add to the old unique ID\
                      (only if the Unique ID is numeric).")
    
    # Tool Output
    output = parser.add_argument_group(description="Output")
    output.add_argument('-w',"--wide", dest="wide", action="store", 
            required=True, help="Wide dataset file.")
    standard.add_argument('-d',"--design", dest="design", action='store', 
            required=True, help="Design file.")
    output.add_argument('-a',"--annot", dest="annot", action="store", 
...
def main(args):
    ...
    """
    This function creates a Wide Dataset, a Design File and an Annotation File 
...
if __name__ == '__main__':
    ...
    logger.info(u"""Importing data with following parameters: \
        \n\tInput: {0}\
        \n\tSamples: {1}"""
        .format(args.input, args.samples))
