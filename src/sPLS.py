...
# DATE: 2018/April/17
#
# MODULE: add_kegg_anno_info.py
#
# VERSION: 1.0
#
# AUTHOR: Francisco Huertas (f.huertas@ufl.edu)
#
# DESCRIPTION: This tool takes the column with the gene and/or metabolites names and
# returns an annotation file with KEGG information (Name in KEGG, KEGG ID, ...)
#
#######################################################################################
...
def getOptions():
    ...
    parser = argparse.ArgumentParser(description="sPLS", formatter_class=RawDescriptionHelpFormatter)

    genes = parser.add_argument_group(description="Gene Expression")
    genes.add_argument('-g',"--geneDataset", dest="geneDataset", action='store',
                      required=True, help="Gene Expression dataset.")
    genes.add_argument('-gid',"--geneId", dest="geneId", action='store',
                      required=True, help="Gene Unique ID column name.")
    genes.add_argument('-ga',"--geneAnno", dest="geneAnno", action='store',
                      required=False, help="Gene Expression Annotation File\
                      (Only if user want gene names in output).")
    genes.add_argument('-gn',"--geneName", dest="geneName", action='store',
                      required=False, help="Gene Names column name in geneAnno File\
                      (Only if user want gene names in output).")
    genes.add_argument('-k',"-keepX", dest="keepX", action='store',
                      required=True, help="Number of genes to keep in each component.")
    genes.add_argument('-t',"-thres", dest="thres", action='store',
                      required=True, help="Threshold to cut the sPLS output file.")
    genes.add_argument('-go',"--geneOption", dest="geneOption", action='store',
                      required=True, help="One of: all, geneList, path, pana.")
    genes.add_argument('-gl',"--geneList", dest="geneList", action='store',
                      required=False, help="Relevant genes to make a sublist\
                      (Only required in geneList option).")
    genes.add_argument('-gkp',"--geneKeggPath", dest="geneKeggPath", action='store',
                      required=False, help="Gene Expression Add KEGG Pathway Info File\
                      (Only required in path option).")
    genes.add_argument('-gka',"--geneKeggAnno", dest="geneKeggAnno", action='store',
                      required=False, help="Gene Expression Add KEGG Annotation Info File\
                      (Only required in pana option).")
    genes.add_argument('-gkn',"--geneKeggName", dest="geneKeggName", action='store',
                      required=False, help="Name of the column in geneKeggAnno that has Gene Symbols\
                      (Only required in pana option).")
    genes.add_argument('-p2g',"--path2genes", dest="path2genes", action='store',
                      required=False, help="Pathway2Genes File, from Add KEGG Pathway Info Tool\
                      (Only required in pana option).")
    genes.add_argument('-p2n',"--path2names", dest="path2names", action='store',
                      required=False, help="PathId2PathNames File, from Add KEGG Pathway Info Tool\
                      (Only required in pana option and if desired).")
    genes.add_argument('-cu',"--cutoff", dest="cutoff", action='store',
                      required=False, help="Variability cut-off value\
                      Default: 0.23. (Only required in pana option).")
    genes.add_argument('-f',"--facSel", dest="facSel", action='store',
                      required=False, help="criterion to select components. One of 'accum', 'single', 'abs.val' or 'rel.abs'\
                      Default: 'single'. (Only required in pana option).")

    metabolites = parser.add_argument_group(description="Metabolites")
    metabolites.add_argument('-m',"--metDataset", dest="metDataset", action='store',
                      required=True, help="Metabolomic Datset.")
    metabolites.add_argument('-mid',"--metId", dest="metId", action='store',
                      required=True, help="Metabolite Unique ID column name.")
    metabolites.add_argument('-ma',"--metAnno", dest="metAnno", action='store',
                      required=False, help="Metabolomics Annotation File\
                      (Only if user want metabolite names in output).")
    metabolites.add_argument('-mn',"--metName", dest="metName", action='store',
                      required=False, help="Metabolite Names column name\
                      (Only if user want metabolite names in output, or in metOption=generic or both).")
    metabolites.add_argument('-mo',"--metOption", dest="metOption", action='store',
                      required=True, help="One of generic, mmc or both.")
    metabolites.add_argument('-mka',"--metKeggAnno", dest="metKeggAnno", action='store',
                      required=False, help="Metabolomic Add KEGG Annotation Info File. (Only required if metOption != mmc).")
    metabolites.add_argument('-mkp',"--metKeggPath", dest="metKeggPath", action='store',
                      required=False, help="Metabolomic Add KEGG Pathway Info File\
                      (Only if geneOption = path).")
    metabolites.add_argument("-d" ,"--design",dest="design", action='store',
                      required=False, help="Design file (Only required in mmc or both option).")

    mmc = parser.add_argument_group(description="MMC")
    mmc.add_argument("-c",'--correlation', dest="correlation", default="pearson",
                        choices=("pearson", "kendall", "spearman"), required=False,
                        help=("Compute correlation coefficients using either "
                        "'pearson' (standard correlation coefficient), "
                        "'kendall' (Kendall Tau correlation coefficient), or "
                        "'spearman' (Spearman rank correlation)."))
    mmc.add_argument("-sl",'--sigmaLow',dest="sigmaLow", type=float, required=False,
                        default=0.05, help="Low value of sigma (Default: 0.05).")
    mmc.add_argument("-sh",'--sigmaHigh',dest="sigmaHigh", type=float, required=False,
                        default=0.50, help="High value of sigma (Default: 0.50).")
    mmc.add_argument("-sn",'--sigmaNum',dest="sigmaNum", type=float, required=False,
    ...
    output.add_argument('-f1', "--figure1", dest="figure1", action="store",
                        required=True, help="sPLS heatmaps")
    output.add_argument('-o1',"--splsOut", dest="splsOut", action="store",
                        required=True, help="Output Table.")
    output.add_argument('-f2', "--figure2", dest="figure2", action="store",
                        required=False, help="MMC Heatmaps (Only if MMC Option)")
    output.add_argument('-o2',"--mmcOut", dest="mmcOut", action="store",
                        required=False, help="MMC Output TSV name (Only if MMC Option)")
    output.add_argument('-o3',"--panaOut", dest="panaOut", action="store",
                        required=False, help="PANA Output TSV name (Only if PANA Option)")
...
def main(args):
    ...
    """
    This function performs an Sparse Partial Least Squares (sPLS) analysis over subsets of gene expression and metabolomic data.
    To perform this subsetting, three different methodologies can be used for the metabolites:
    ...
    # Establish path for sPLS script
    ...
    # Load the R function containing sPLS
    with open(my_r_script_path, 'r') as f:
        rFile = f.read()

    # name of sPLS function is sPLS
    ...
    for R_met_df in rMetData:
        ...
        rData.append(sPLSScript.sPLS(geneData=R_gene_df, metData=R_met_df, keepX=args.keepX))
        if args.geneOption == 'path':
            data_counter += 1

    # Plot Heatmap
    if args.metOption == 'both':
        sPLSScript.plotInPdf(splsObjects=rData, figurePath=args.figure1, multipleNames=multipleNamesId)
        # Correlation Matrix
        corMatrix = sPLSScript.corrMat(splsObjects=rData, multipleNames=multipleNamesId, threshold=args.thres)
        robjects.r['write.table'](corMatrix, file = args.splsOut, sep = '\t', quote = False, row_names = False, col_names = True)
    else:
        sPLSScript.plotInPdf(splsObjects=rData, figurePath=args.figure1, multipleNames=multipleNames)
        # Correlation Matrix
        corMatrix = sPLSScript.corrMat(splsObjects=rData, multipleNames=multipleNames, threshold=args.thres)
        robjects.r['write.table'](corMatrix, file = args.splsOut, sep = '\t', quote = False, row_names = False, col_names = True)
...
if __name__ == '__main__':
    ...
    logger.info(u"Importing data with the following parameters: "
                ...
                "\n\tMetabolite UniqueID:  {}"
                "\n\tMetabolite Option: {}".
                format(args.geneDataset, args.geneId, args.geneOption, args.metDataset, args.metId, args.metOption))
