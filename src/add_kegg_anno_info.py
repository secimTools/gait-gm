...
# returns an annotation file with linked KEGG information (Name in KEGG, KEGG ID, ...)
#
#######################################################################################
...
def getOptions():
    ...
    tool.add_argument('-s',"--species", dest="species", action='store', 
            required=True, help="Specie to download.")
    tool.add_argument('-ga',"--geneAnnot", dest="geneAnnot", action='store', 
            required=False, help="Gene Annotation File.")
    ...
    tool.add_argument('-ma',"--metAnnot", dest="metAnnot", action='store', 
            required=False, help="Metabolite Annotation File.")
...
def main(annotFile, uniqId, featureNames, parser, outputFile, featureType):
    ...
    """
    Function to analyze input genes or metabolites, finding the correspondent name in kegg and its identifier.
    It selects the best match in similarity terms and discard the other possible ocurrences.
    Informs about ties to facilitate user's correction.
    ...
    output.write(uniqId + "\t" + featureNames + "\tFeature_Type\tMatched\tName_in_KEGG\tKEGG_ID\tSimilarity\tTie\tSelected\n") 
...
if __name__ == '__main__':
    ...
    if args.geneAnnot:
        ...
        main(args.geneAnnot, args.geneUniqId, args.geneName, gene2keggs, args.geneOut, featureType = 'Gene')
    if args.metAnnot:
        modules.checkForDuplicates(args.metAnnot, args.metUniqId)
        met2keggs = modules.downloadMetParser()
        main(args.metAnnot, args.metUniqId, args.metName, met2keggs, args.metOut, featureType = 'Metabolite')
